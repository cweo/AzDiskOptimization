...
# Example usage (after Connect-AzAccount):
...
from modules.compute import get_tier, get_lower_recommended_tier
...
parser.add_argument("--drop_nan", action="store_true", help="Drop rows with NaN values.")
...
def main(args):
    ...
    if path_throughout_IOPS.is_file() and args.use_disks_df:
        ...
        disks_throughput_IOPS_df = pd.read_csv("data/diskMetrics.csv")
    ...
    disks_df = disks_throughput_IOPS_df[["id", "sku", "tier", "size", "location"]]
    disks_df.rename(columns={"sku": "sku_name",
                             "tier": "tier_name", "size": "disk_size_gb"}, inplace=True)
    ...
    # print(disks_tier_size_df.head(), disks_throughput_IOPS_df.head())
    disks_metrics_tier = disks_tier_size_df.merge(
        disks_throughput_IOPS_df[["id", "IOPS", "throughput"]], on="id")
    ...
    if (args.drop_nan):
        disks_redundancy_recommendation_df.dropna(inplace=True)
